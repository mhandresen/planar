...
- **Demotes security groups.** Folds security groups, NSGs, and firewalls into a shield badge on the resources they protect — conservatively, so a _changed_ group is never silently dropped.
...
Run Planar in CI to keep an architecture diagram of your infrastructure committed to your repo, updated on every merge to your default branch:
...
permissions:
  contents: write # Planar commits the diagram back to the repo
...
jobs:
  diagram:
    ...
    steps:
      ...
      # Authenticate to your cloud as you normally would in CI
      # (e.g. aws-actions/configure-aws-credentials via OIDC), then:
      - run: terraform init
      - run: terraform plan -out tfplan.bin
      - run: terraform show -json tfplan.bin > plan.json

      - uses: mhandresen/planar@v1
        with:
          plan: plan.json
          output: .planar/diagram.svg
          theme: light
```

> **Note:** `permissions: contents: write` is required — without it, the commit step fails with a 403.

### Inputs

| Input            | Default                        | Description                                                  |
| ---------------- | ------------------------------ | ------------------------------------------------------------ |
| `plan`           | _(required)_                   | Path to a `terraform show -json` plan document.              |
| `output`         | `.planar/diagram.svg`          | Where to write the SVG, relative to the repository root.     |
| `theme`          | `light`                        | Diagram theme: `light` or `dark`.                            |
| `commit-message` | `chore: update Planar diagram` | Message used when the diagram changes on the default branch. |
| `token`          | `${{ github.token }}`          | Token used to commit the diagram.                            |

On a push to your default branch, Planar renders the diagram and commits it (only when it has changed). On other events it writes the SVG without committing — posting the diff to pull requests is on the roadmap.

### Pull request comments

To post the diagram as a comment on pull requests, add a `pull_request` trigger
and the `pull-requests: write` permission:
...
permissions:
  contents: write # commit the diagram on merge to the default branch
  pull-requests: write # post the diagram comment on PRs
```

On a pull request, Planar renders the diagram, hosts it on a `planar-bot` branch,
and posts (or updates) a single sticky comment with the diff. Public repositories
only — pull requests from forks get a read-only token and are skipped.

### Why is `dist/` committed?

GitHub Actions run the bundled file directly with no install step, so the
compiled Action (`dist/index.js` plus its wasm and font assets) is committed
on purpose. It's regenerated with `pnpm run bundle`, and CI fails if it drifts
...
- [x] **M3** — GitHub Action: post the diff on PRs, commit/update the SVG on merge.
- [ ] **M4** — Module-aware references: walk `configuration` recursively through `module_calls` so cross-module references resolve. Restores nesting and security-group demotion on modular plans (most real-world Terraform).

## Known limitations & TODO

**Post-launch**

- **Module-aware grouping.** Reference resolution currently reads the root module only, so on modular plans resources group by module and cross-module containment is lost (e.g. a subnet renders separately from the vnet it belongs to in another module), and security-group demotion falls back to a node instead of a badge. The output is legible but flatter than a single-module plan. Fixed by M4.
- **Multi-provider depth.** The pipeline is provider-general and runs on Azure, but coverage is AWS-first: bespoke glyphs are AWS-only (Azure/GCP fall back to per-category glyphs, so distinct network kinds can share a glyph), some keywords are missing (e.g. `azurerm_public_ip` lands in "other"), Azure resource-group containment isn't modeled, and GCP is seeded but untested. Add Azure/GCP fixtures and tests to lock it in.
